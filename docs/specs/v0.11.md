...
发布工作流只接受位于 `main` 的注解式稳定标签，且标签必须精确等于 workspace 版本。Windows/Linux CLI 在同一 runner 的两个全新 target 目录分别构建，字节不一致立即拒绝；确定性 ZIP、构建记录、规范化 CycloneDX 1.5 SBOM、release manifest 和 `SHA256SUMS` 共同形成闭包，最终由 GitHub OIDC 生成 keyless provenance。详细威胁模型、验证命令和可复现性声明见 [release-supply-chain.md](../engineering/release-supply-chain.md)。
