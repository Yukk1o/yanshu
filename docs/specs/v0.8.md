...
Status: implemented Rust language contract. Guest language version: `4`.
...
`rust-readonly-v1` is the historical renderer identifier introduced by v0.8. The current v0.12 implementation emits `rust-readonly-v3`, which adds explicit effect-call and truthiness/BigInt annotations without making the projection editable.
