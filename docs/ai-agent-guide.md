...
这份文件同时定义两件事：Codex、Claude Code、OpenCode 如何参与开发这个仓库，以及 AI-Evolve 如何把它们作为候选程序的 Agent Backend 调用。根目录的 `AGENTS.md` 与 `CLAUDE.md` 只负责自动发现；项目约束只在这里维护，避免不同代理得到互相矛盾的规则。
...
```powershell
$env:AI_EVOLVE_PROVIDER = "codex-cli"       # 或 claude-code-cli / opencode-cli
...
active 源码 + 可信测试报告
              ↓
一次性候选目录（candidate.ail / OBSERVATIONS.json / TASK.md）
              ↓
...
AI-Evolve 是“程序即数据”的受限通用语言内核。AI 可以生成候选程序，但候选必须经过 Parser、类型/效果分析、capability 闭包、fuel、Schema、内容哈希和测试门禁，才允许执行或晋升。

当前语言版本是 v4，项目发布里程碑是 v0.10。Rust 是唯一主实现；第一方 crate 均要求 `#![forbid(unsafe_code)]`。
...
- `.ail` 源码、Bundle manifest、package source 与 lock 是规范输入；生成的 Rust 风格审查文本不是 Rust 源码，也不能反向执行。

## 2. 仓库地图

- `rust/crates/ail-syntax`：Reader、AST、版本门禁和 Parser。
- `rust/crates/ail-runtime`：解释器、portable value、Schema、pattern 和字节码 VM。
- `rust/crates/ail-analysis`：静态类型、效果、capability 闭包与只读审查投影。
- `rust/crates/ail-compiler`：规范字节码、verifier、artifact 和 WASM handle ABI。
- `rust/crates/ail-bundle`、`ail-package`：密封模块链接、内容寻址包与锁文件。
- `rust/crates/ail-library`：有版本、有契约、有计费规则的 Rust Library Backend。
- `rust/crates/ail-cli`：面向人类和代理的稳定 JSON CLI。
...
- capability 调用在审查视图中显示为 `log!` 一类效果标记；源码中仍按 `.ail` 语法书写；
...
```powershell
cargo run --locked -p ail-cli -- inspect examples\expenses\service.ail
cargo run --locked -p ail-cli -- review-bundle examples\bundles\typed-expense
cargo run --locked -p ail-cli -- review-bundle examples\bundles\typed-expense --text
```

运行 v1 至 v4 的可执行契约：

```powershell
cargo run --locked -p ail-cli -- conformance conformance\v1\manifest.json
cargo run --locked -p ail-cli -- conformance conformance\v2\manifest.json
cargo run --locked -p ail-cli -- conformance conformance\v3\manifest.json
cargo run --locked -p ail-cli -- conformance conformance\v4\manifest.json
```
...
现在同时支持两条路径：代理进入真实仓库参与语言实现；以及 `evolve-service` 在一次性候选目录中调用 Codex/Claude Code/OpenCode 编写一个 `.ail` 候选。后者默认只登记，不晋升，且不会把 agent 的退出状态或 notes 当成通过证据。

当前尚未提供 `.ail` LSP、Tree-sitter grammar、编辑器插件、MCP server 或审查视图的结构化回写。这些属于下一层开发体验，不能用不可靠的文本反向转换冒充。
