...
当前 formatter v1 已提供只读候选输出、CI check 和不依赖 source offset 的表达式节点路径；`yanshu-syntax` 已提供全局 `def`、参数、顺序 `let`、pattern binding 和嵌套遮蔽的有界符号索引；最小 `yanshu-lsp` 已提供 full sync、诊断、hover、同文件全局/局部 definition、references 和只读 formatting edit；VS Code 扩展已提供 `.yan` 注册、基础 TextMate 高亮、平台专用 VSIX，以及隔离的 Windows/Linux Extension Host 验收。尚未提供 Tree-sitter grammar、其它编辑器安装包、MCP server、LSP 局部增量/rename/completion 或审查视图的结构化回写；这些不能用不可靠的文本反向转换冒充。
