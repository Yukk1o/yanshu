...
v3 的字段只有名字；v4 的每个字段都必须有类型。模式构造器、字段数量和分支结果会参与统一，错误带回原 `.yan` span。
...
`!` 是审查标记，不是第二套可执行语法。`log!(...)` 表示该行直接调用 capability；`typed_policy__audit!(...)` 表示调用会传递性到达 capability。机器节点仍保留精确 `.yan` span 和 capability 列表。
