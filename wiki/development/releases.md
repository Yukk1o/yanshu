...
::: warning 当前范围
当前公开里程碑仍是 v0.10；发布链正在 v0.11 开发。它只覆盖 Windows x86-64 与 Linux x86-64 的 `yanshu` CLI，不代表 crates.io、macOS、ARM、安装器或生产稳定性。
:::
...
每个平台有一个 ZIP 和一个 `.build.json`。构建记录写明 target、源码 commit、源码时间、Rust/Cargo 实际版本、二进制/归档大小与 SHA-256。总 release manifest 再把两个平台、SBOM 和构建记录闭合，`SHA256SUMS` 覆盖所有 payload 与 manifest。
...
同一平台 job 在两个全新的 Cargo target 目录完整构建，只有可执行文件逐字节相同才继续。它固定 Cargo lock、Rust patch 工具链、`SOURCE_DATE_EPOCH`、源码路径映射与 release profile。
