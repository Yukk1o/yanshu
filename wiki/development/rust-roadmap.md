...
| 工具 | check/inspect/review、只读 formatter v1/CI check、有界全局/局部符号索引、LSP 诊断/精确 token hover/作用域感知 completion/同文档 definition/references/formatting edit/版本化 review、VS Code 无脚本审查面板、平台专用 VSIX、Windows/Linux Extension Host 验收、Bundle、package pack/lock/verify/review/run/compile、bytecode/WASM compile/inspect/run、conformance、test/deploy/evolve service、version lifecycle、跨平台 CI、边界 fuzz、双构建与 keyless provenance | Tree-sitter、LSP 增量/rename、跨文件导航、更多编辑器、MCP、结构化 AST diff、操作型 rollback CLI |
...
7. **v0.12**：Agent 工具协议、formatter、Tree-sitter、LSP 与 MCP；formatter v1、稳定节点路径、有界全局/局部符号索引、精确 token hover、作用域感知 completion、同文档 definition/references、版本化只读审查面板、VS Code 平台包与 Windows/Linux Extension Host 验收已经落地，其余仍在开发；
