...
| 层 | 已实现 | 尚未完成 |
| --- | --- | --- |
| 语言前端 | 有边界 UTF-8 Reader、v1-v4 AST / Parser、imports、typed 封闭数据、模式匹配、signature、版本门控、source span、JSON inspect、稳定 expression-v1 节点路径、最小 LSP full sync、只读 Tree-sitter 增量 CST 与标准查询 | 泛型声明、规范 AST 的增量重建、跨任意结构编辑的节点 identity reconciliation |
...
| 工具 | check/inspect/review、只读 formatter v1/CI check、有界全局/局部符号索引、LSP 诊断/精确 token hover/作用域感知 completion/全文 semantic tokens/同文档 definition/references/防捕获 rename/formatting edit/版本化 review、VS Code 无脚本审查面板、平台专用 VSIX、Windows/Linux Extension Host 验收、Tree-sitter grammar/corpus/query/差分门禁、只读 MCP inspect/format/review、Bundle、package pack/lock/verify/review/run/compile、bytecode/WASM compile/inspect/run、conformance、test/deploy/evolve service、version lifecycle、跨平台 CI、边界 fuzz、双构建与 keyless provenance | semantic token range/delta、LSP 增量、跨文件导航、更多编辑器、结构化 AST diff、操作型 rollback CLI |
...
7. **v0.12**：Agent 工具协议、formatter、Tree-sitter、LSP 与 MCP；formatter v1、稳定节点路径、有界全局/局部符号索引、精确 token hover、作用域感知 completion、全文 semantic tokens、同文档 definition/references、防捕获 rename、版本化只读审查面板、VS Code 平台包与 Windows/Linux Extension Host 验收、只读 Tree-sitter grammar/corpus/query/差分门禁，以及兼容现代/旧版客户端的只读 MCP 已经落地；其它编辑器与结构化编辑仍在开发；
