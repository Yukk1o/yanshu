...
| 工具 | check/inspect/review、只读 formatter v1/CI check、有界局部符号索引、LSP 诊断/hover/全局与局部 definition/formatting edit、平台专用 VS Code VSIX、Windows/Linux Extension Host 验收、Bundle、package pack/lock/verify/review/run/compile、bytecode/WASM compile/inspect/run、conformance、test/deploy/evolve service、version lifecycle、跨平台 CI、边界 fuzz、双构建与 keyless provenance | Tree-sitter、LSP 增量/references/rename/completion、更多编辑器、MCP、结构化 AST diff、操作型 rollback CLI |
...
7. **v0.12**：Agent 工具协议、formatter、Tree-sitter、LSP 与 MCP；formatter v1、稳定节点路径、有界局部符号索引、全局/局部 definition、VS Code 平台包与 Windows/Linux Extension Host 验收已经落地，其余仍在开发；
