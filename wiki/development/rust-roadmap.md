...
| 工具 | check/inspect/review、只读 formatter v1/CI check、LSP 诊断/hover/全局 definition/formatting edit、平台专用 VS Code VSIX、Bundle、package pack/lock/verify/review/run/compile、bytecode/WASM compile/inspect/run、conformance、test/deploy/evolve service、version lifecycle、跨平台 CI、边界 fuzz、双构建与 keyless provenance | Tree-sitter、LSP 增量/局部导航/rename/completion、更多编辑器、MCP、结构化 AST diff、操作型 rollback CLI |
...
7. **v0.12**：Agent 工具协议、formatter、Tree-sitter、LSP 与 MCP；formatter v1、稳定节点路径、最小 LSP 与 VS Code 平台包已经落地，其余仍在开发；
