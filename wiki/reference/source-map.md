...
| [lsp/semantic_tokens/classify.rs](/source/rust/crates/yanshu-lsp/src/semantic_tokens/classify.rs.txt) | AST/Reader/符号索引驱动的关键词、类型、绑定、构造器与内建分类 |
...
| [VS Code Extension Host suite](/source/editors/vscode/src/test/suite/index.ts.txt) | 激活、诊断、补全、semantic tokens、导航、rename、formatting 与无文本模型审查面板的端到端断言 |
...
| [Release workflow](/source/.github/workflows/release.yml.txt) | 标签身份、CLI/MCP/LSP 与 VSIX 双构建、四份 SBOM、checksum、keyless provenance 与发布权限 |
| [发布构建器](/source/scripts/build-release.mjs.txt) | clean source 绑定、三个 Rust 程序双 target-dir 字节比对、真实 smoke、双 VSIX、MSVC `/Brepro` 与确定性共享 ZIP |
| [Rust SBOM 归一化器](/source/scripts/normalize-sbom.mjs.txt) | Cargo 图的 commit 身份、确定时间与 checkout 路径清理 |
| [扩展 SBOM 归一化器](/source/scripts/normalize-extension-sbom.mjs.txt) | npm 生产图、规范根组件与内含 LSP 关系 |
| [发布组装器](/source/scripts/assemble-release.mjs.txt) | 两个平台、三个程序、两个 VSIX、四份 SBOM、构建记录、schema v3 manifest 与 SHA-256 闭包 |
| [发布验证器](/source/scripts/verify-release.mjs.txt) | 下载目录的精确资产集、四份 SBOM、checksum、大小和 manifest 覆盖校验 |
