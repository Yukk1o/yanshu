...
AI-Evolve v0.8 已有用户模块、typed 封闭数据、模式匹配、密封 Bundle、导出签名、静态 capability 闭包和 Rust 风格只读审查，但仍处于通用语言的安全内核阶段，还不是通用系统语言或公网生产框架。它尚无跨 Bundle 包管理、编译目标、并发、浮点数及通用文件/网络 API；宿主侧也仍缺少独立进程沙箱、正式数据库/PITR、异地备份、指标告警和 canary 自动化。
