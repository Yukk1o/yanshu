  ...
  (unless (file-exists? metadata-path)
    (write-json-file
     ...
     (hasheq 'hash hash
             ...
             'program (symbol->string (ail-program-name program))
             'languageVersion (ail-program-version program)
             'provider provider
...
(define (promote! root hash)
  ...
  (unless (hash-ref report 'passed (lambda () #f))
    (raise-ail "VERSION_TESTS_NOT_PASSED"
  ...
  (unless (equal? parent current)
    (raise-ail "VERSION_PARENT_MISMATCH"
...
(define (version-source-path root hash)
  (build-path root "versions" (string-append hash ".ail")))
