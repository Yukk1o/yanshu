...
use ail_diagnostic::{AilResult, Diagnostic, Position, Span};
...
impl<'source> Reader<'source> {
    ...
    fn consume_node(&mut self, depth: usize, start: Position) -> AilResult<()> {
        self.node_count += 1;
    ...
    fn read_datum(&mut self, depth: usize) -> AilResult<Datum> {
        self.skip_ignored();
    ...
    fn read_list(&mut self, depth: usize, open: char, close: char) -> AilResult<Datum> {
        let start = self.position();
    ...
    fn read_quote(&mut self, depth: usize) -> AilResult<Datum> {
        let start = self.position();
    ...
    fn read_string(&mut self) -> AilResult<Datum> {
        let start = self.position();
    ...
    fn read_atom(&mut self) -> AilResult<Datum> {
        let start = self.position();
