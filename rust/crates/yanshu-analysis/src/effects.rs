...
fn collect_pattern_bindings(pattern: &yanshu_syntax::Pattern, bindings: &mut BTreeSet<String>) {
    match &pattern.kind {
        yanshu_syntax::PatternKind::Binding(name) => {
            bindings.insert(name.clone());
        }
        yanshu_syntax::PatternKind::Variant { fields, .. } => {
            for field in fields {
                collect_pattern_bindings(field, bindings);
            }
        }
        yanshu_syntax::PatternKind::Wildcard | yanshu_syntax::PatternKind::Literal(_) => {}
    }
}
