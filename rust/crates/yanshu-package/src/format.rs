...
pub(crate) fn object<'a>(value: &'a Value, kind: &str) -> YanshuResult<&'a Map<String, Value>> {
...
pub(crate) fn string<'a>(value: &'a Map<String, Value>, field: &str) -> YanshuResult<&'a str> {
    value
        .get(field)
...
pub(crate) fn u64_field(value: &Map<String, Value>, field: &str) -> YanshuResult<u64> {
    value.get(field).and_then(Value::as_u64).ok_or_else(|| {
        Diagnostic::simple("PACKAGE_INVALID_DOCUMENT", "required integer is missing")
    })
}

pub(crate) fn array<'a>(value: &'a Map<String, Value>, field: &str) -> YanshuResult<&'a [Value]> {
    value
        .get(field)
...
pub(crate) fn valid_relative_path(value: &str, yanshu_only: bool) -> bool {
    ...
    !path.is_absolute()
        ...
        && (!yanshu_only || path.extension().and_then(|item| item.to_str()) == Some("yan"))
}
