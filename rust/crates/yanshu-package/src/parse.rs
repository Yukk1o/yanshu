...
pub(crate) fn package_manifest(document: &Value) -> YanshuResult<PackageManifest> {
...
pub(crate) fn package_lock(document: &Value) -> YanshuResult<PackageLock> {
...
fn package_dependencies(values: &[Value]) -> YanshuResult<Vec<PackageDependency>> {
    let mut dependencies = Vec::new();
...
fn string_list(values: &[Value], paths: bool, maximum: usize) -> YanshuResult<Vec<String>> {
...
fn bounded(values: &[Value], maximum: usize) -> YanshuResult<&[Value]> {
...
fn checked_name(value: &str) -> YanshuResult<String> {
    if valid_name(value) {
        Ok(value.to_owned())
    } else {
        Err(Diagnostic::simple(
...
fn checked_version(value: &str) -> YanshuResult<String> {
...
fn checked_hash(value: &str) -> YanshuResult<String> {
    if valid_hash(value) {
        Ok(value.to_owned())
    } else {
        Err(Diagnostic::simple(
...
fn checked_path(value: &str, yanshu_only: bool) -> YanshuResult<String> {
    if valid_relative_path(value, yanshu_only) {
        Ok(value.to_owned())
    } else {
        Err(Diagnostic::simple(
