...
pub(crate) fn validate_code_store(
    root: &Path,
    allow_lock: bool,
) -> AilResult<(Option<String>, Vec<SnapshotFile>)> {
...
pub(crate) fn enforce_snapshot_limits(files: &[SnapshotFile]) -> AilResult<()> {
    if files.len() > MAXIMUM_FILES {
        return Err(backup_too_large());
    }
    let mut total = 0_u64;
...
fn read_snapshot_file(path: &Path, relative: &str) -> AilResult<SnapshotFile> {
    Ok(SnapshotFile {
...
fn metadata_parent(store: &VersionStore, hash: &str) -> AilResult<Option<String>> {
