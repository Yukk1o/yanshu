...
use ail_http::ShadowControls;
use ail_rollout::{JsonlShadowObservationSink, ShadowObservationSink, ShadowPolicy, ShadowRuntime};
...
pub fn configured_shadow(
    code_store: &str,
    data_store: &str,
) -> AilResult<Option<ConfiguredShadow>> {
    let version = optional_variable("AI_EVOLVE_SHADOW_VERSION")?;
    let percent = optional_variable("AI_EVOLVE_SHADOW_PERCENT")?;
    let concurrency = optional_variable("AI_EVOLVE_SHADOW_MAX_CONCURRENCY")?;
    let Some(version) = version else {
        if percent.is_some() || concurrency.is_some() {
            return Err(invalid_shadow_config(
                "shadow percentage or concurrency requires AI_EVOLVE_SHADOW_VERSION",
            ));
        }
        return Ok(None);
    };
    let percent = percent.ok_or_else(|| {
        invalid_shadow_config(
            "AI_EVOLVE_SHADOW_PERCENT is required when a shadow version is configured",
        )
    })?;
    let percent = percent.parse::<u8>().map_err(|_| {
        invalid_shadow_config("AI_EVOLVE_SHADOW_PERCENT must be an integer from 1 to 100")
    })?;
    if percent == 0 || percent > 100 {
        return Err(invalid_shadow_config(
            "AI_EVOLVE_SHADOW_PERCENT must be an integer from 1 to 100",
        ));
    }
    let maximum_concurrency = concurrency.map_or(Ok(4_usize), |value| {
        value.parse::<usize>().map_err(|_| {
            invalid_shadow_config("AI_EVOLVE_SHADOW_MAX_CONCURRENCY must be a positive integer")
        })
    })?;
    if maximum_concurrency == 0 {
        return Err(invalid_shadow_config(
            "AI_EVOLVE_SHADOW_MAX_CONCURRENCY must be a positive integer",
        ));
    }
...
fn optional_variable(name: &'static str) -> AilResult<Option<String>> {
    match env::var(name) {
