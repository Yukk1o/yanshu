...
pub fn acquire_service_lease(data_store: impl AsRef<Path>) -> YanshuResult<ServiceLease> {
...
pub(crate) fn acquire_version_lease(code_store: &Path) -> YanshuResult<File> {
    validate_directory(code_store)?;
    let path = code_store.join(".yanshu-store.lock");
    let file = OpenOptions::new()
        .read(true)
        .write(true)
        .create(true)
