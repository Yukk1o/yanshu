...
pub(crate) fn read_bounded(path: &Path, maximum: u64) -> YanshuResult<Vec<u8>> {
...
pub(crate) fn validate_regular_file(path: &Path, maximum: u64) -> YanshuResult<()> {
...
pub(crate) fn regular_file_present(path: &Path, maximum: u64) -> YanshuResult<bool> {
...
pub(crate) fn validate_directory(path: &Path) -> YanshuResult<()> {
    let metadata = fs::symlink_metadata(path).map_err(|_| invalid_backup_path(path))?;
...
pub(crate) fn read_directory(path: &Path) -> YanshuResult<Vec<fs::DirEntry>> {
...
pub(crate) fn write_new_file(path: &Path, bytes: &[u8]) -> YanshuResult<()> {
...
pub(crate) fn create_temporary_directory(destination: &Path) -> YanshuResult<PathBuf> {
...
pub(crate) fn collect_payload_paths(root: &Path) -> YanshuResult<BTreeSet<String>> {
...
pub(crate) fn validate_manifest_path(relative: &str) -> YanshuResult<()> {
...
pub(crate) fn join_safe_relative(root: &Path, relative: &str) -> YanshuResult<PathBuf> {
    validate_safe_relative(relative)?;
    Ok(relative
        .split('/')
...
fn validate_safe_relative(relative: &str) -> YanshuResult<()> {
...
pub(crate) fn reject_existing(path: &Path, code: &'static str) -> YanshuResult<()> {
    match fs::symlink_metadata(path) {
...
pub(crate) fn reject_destination_inside_source(
    source: &Path,
    destination: &Path,
) -> YanshuResult<()> {
...
fn future_absolute_path(path: &Path) -> YanshuResult<PathBuf> {
...
pub(crate) fn file_name(path: &Path) -> YanshuResult<String> {
    path.file_name()
        .and_then(|value| value.to_str())
        .map(str::to_owned)
