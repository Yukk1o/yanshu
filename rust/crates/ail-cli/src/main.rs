...
use ail_provider::{EvolutionProvider, EvolutionRequest, configured_evolution_provider};
use ail_runtime::{
...
impl CapabilityHost for CliCapabilityHost {
    ...
    fn invoke(&mut self, operation: &str, _arguments: &[GuestValue]) -> AilResult<GuestValue> {
        if operation != "log" {
...
fn read_program_file(path: &str) -> AilResult<Program> {
    let source = fs::read_to_string(path).map_err(|error| {
        Diagnostic::new(
            "HOST_FILE_READ",
...
fn read_artifact_file(path: &str, artifact: &str) -> AilResult<Vec<u8>> {
...
fn read_agent_task(path: &str) -> AilResult<String> {
...
mod tests {
    ...
    fn reviews_and_runs_a_typed_sealed_bundle() {
        ...
        let bytecode = temporary.path.join("bundle.aibc.json");
    ...
    fn locks_reviews_and_runs_a_content_addressed_package() {
        ...
        let bytecode = temporary.path.join("typed-expense.aibc.json");
