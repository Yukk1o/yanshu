...
use ail_compiler::{CodeBlock, Instruction};
...
fn peek_bytecode_value(stack: &[Value], pc: usize) -> AilResult<&Value> {
    stack.last().ok_or_else(|| bytecode_stack_underflow(pc))
}

fn pop_bytecode_value(stack: &mut Vec<Value>, pc: usize) -> AilResult<Value> {
