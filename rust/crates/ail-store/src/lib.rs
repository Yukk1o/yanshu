...
impl VersionStore {
    ...
    pub fn register_candidate(&self, registration: CandidateRegistration<'_>) -> AilResult<String> {
    ...
    pub fn promote(&self, hash: &str, at: u64) -> AilResult<String> {
        validate_hash(hash)?;
        self.with_write_lock(|| self.promote_unlocked(hash, at))
    }

    pub fn rollback(&self, at: u64) -> AilResult<String> {
        self.with_write_lock(|| self.rollback_unlocked(at))
    }

    pub fn active_hash(&self) -> AilResult<Option<String>> {
        self.active_hash_unlocked()
    }

    pub fn active_source(&self) -> AilResult<String> {
    ...
    pub fn version_source(&self, hash: &str) -> AilResult<String> {
        validate_hash(hash)?;
        self.version_source_unlocked(hash)
    }

    pub fn version_metadata(&self, hash: &str) -> AilResult<JsonValue> {
        validate_hash(hash)?;
        self.version_metadata_unlocked(hash)
    }

    fn with_write_lock<T>(&self, operation: impl FnOnce() -> AilResult<T>) -> AilResult<T> {
        fs::create_dir_all(&self.root).map_err(|_| lock_failure(&self.root))?;
        let lock_path = self.root.join(".ail-store.lock");
    ...
    fn promote_unlocked(&self, hash: &str, at: u64) -> AilResult<String> {
    ...
    fn rollback_unlocked(&self, at: u64) -> AilResult<String> {
    ...
    fn active_hash_unlocked(&self) -> AilResult<Option<String>> {
    ...
    fn version_source_unlocked(&self, hash: &str) -> AilResult<String> {
    ...
    fn version_metadata_unlocked(&self, hash: &str) -> AilResult<JsonValue> {
    ...
    fn ensure_directories(&self) -> AilResult<()> {
    ...
    fn write_active_pointer(&self, hash: &str) -> AilResult<()> {
        self.ensure_directories()?;
        write_json_atomically(&self.root.join("active.json"), &json!({ "active": hash }))
    }

    fn append_event(&self, event: &JsonValue) -> AilResult<()> {
        self.ensure_directories()?;
    ...
    fn version_source_path(&self, hash: &str) -> PathBuf {
        self.root.join("versions").join(format!("{hash}.ail"))
    }
...
fn validate_hash(hash: &str) -> AilResult<()> {
    if hash.len() == 64
        && hash
            .bytes()
...
mod tests {
    ...
    fn error_code<T>(result: AilResult<T>) -> &'static str {
        match result {
    ...
    fn rejects_path_traversal_and_detects_source_tampering() {
        ...
        fs::write(
            temporary.path.join("versions").join(format!("{hash}.ail")),
            CANDIDATE_SOURCE,
        )
    ...
    fn cross_process_lock_fails_with_a_bounded_timeout() {
        let temporary = TestDirectory::new();
        let lock_path = temporary.path.join(".ail-store.lock");
        let lock = fs::OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
