...
const WASM_TARGET: &str = "ail-wasm-bytecode-v1";
const WASM_META_SECTION: &str = "ail.meta.v1";
const WASM_BYTECODE_SECTION: &str = "ail.bytecode.v1";
const AIL_WASM_ABI_FORMAT_VERSION: u8 = 1;
...
impl WasmArtifact {
    ...
    pub fn to_json(&self) -> Value {
        json!({
            "target": WASM_TARGET,
            "abiFormatVersion": AIL_WASM_ABI_FORMAT_VERSION,
            ...
            "imports": ["ail_v1.execute"],
            "exports": ["ail_format_version", "ail_static_instruction_weight", "ail_run"],
            "abi": "ail_run(export_index: i32, arguments_handle: i32, fuel: i64) -> result_handle: i64",
            "guestExports": self.guest_exports,
            "execution": "trusted-ail-bytecode-vm",
        })
    }
}

pub fn compile_wasm(bytecode: &BytecodeArtifact) -> AilResult<WasmArtifact> {
...
pub fn write_wasm_artifact(artifact: &WasmArtifact, path: impl AsRef<Path>) -> AilResult<()> {
...
pub fn load_wasm_bytecode(program: &Program, source: &[u8]) -> AilResult<BytecodeArtifact> {
...
fn append_import_section(module: &mut Vec<u8>) {
    ...
    append_name(&mut payload, "ail_v1");
...
fn append_export_section(module: &mut Vec<u8>) {
    ...
    append_name(&mut payload, "ail_format_version");
    payload.extend([0x00, 0x01]);
    append_name(&mut payload, "ail_static_instruction_weight");
    payload.extend([0x00, 0x02]);
    append_name(&mut payload, "ail_run");
    payload.extend([0x00, 0x03]);
    append_section(module, 7, &payload);
}

fn append_code_section(module: &mut Vec<u8>, static_weight: u64) -> AilResult<()> {
    let mut payload = Vec::new();
    encode_u64(3, &mut payload);

    let version_body = [0x00, 0x41, AIL_WASM_ABI_FORMAT_VERSION, 0x0b];
    ...
    let run_body = [
        ...
        0x10, 0x00, // call imported ail_v1.execute
        0x0b, // end
    ];
...
fn append_custom_section(module: &mut Vec<u8>, name: &str, content: &[u8]) -> AilResult<()> {
    let mut payload = Vec::new();
