...
mod tests {
    ...
    impl TestDirectory {
        fn new() -> Self {
            ...
            let path = std::env::temp_dir()
                .join(format!("ail-package-test-{}-{nonce}", std::process::id()));
    ...
    fn locks_loads_and_executes_only_content_addressed_sources() {
        ...
        let lock_path = root.join("ail.lock.json");
        fs::create_dir_all(&policy).unwrap_or_else(|error| panic!("{error}"));
        fs::write(root.join("app.ail"), APP).unwrap_or_else(|error| panic!("{error}"));
        fs::write(policy.join("policy.ail"), POLICY).unwrap_or_else(|error| panic!("{error}"));
        fs::write(
            root.join("ail-package.source.json"),
            r#"{
              "formatVersion": 1,
              "name": "expense-app",
              "version": "1.0.0",
        ...
        fs::write(
            policy.join("ail-package.source.json"),
            r#"{
              "formatVersion": 1,
              "name": "policy-lib",
              "version": "1.2.0",
        ...
        fs::write(
            store.join("sha256").join(policy_hash).join("policy.ail"),
            "tampered",
        )
        .unwrap_or_else(|error| panic!("{error}"));
        let diagnostic = match load_locked_package(&store, &lock_path) {
    ...
    fn rejects_source_dependency_path_escape() {
        ...
        .unwrap_or_else(|error| panic!("{error}"));
        let diagnostic =
            match lock_workspace(&root, temporary.0.join("store"), root.join("ail.lock.json")) {
                Err(diagnostic) => diagnostic,
