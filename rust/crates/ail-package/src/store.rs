...
use ail_bundle::link_program_set;
...
const SOURCE_DESCRIPTOR: &str = "ail-package.source.json";
...
pub fn pack_workspace(root: impl AsRef<Path>, store: impl AsRef<Path>) -> AilResult<String> {
...
pub fn verify_package(store: impl AsRef<Path>, content_hash: &str) -> AilResult<PackageManifest> {
    Ok(verify_artifact(&absolute_path(store.as_ref())?, content_hash)?.manifest)
}

impl BuildContext {
    fn build(&mut self, root: &Path) -> AilResult<&BuiltPackage> {
...
fn read_source_descriptor(root: &Path) -> AilResult<SourceDescriptor> {
...
) -> AilResult<(Vec<PackageModule>, ModuleSources, Programs, u64)> {
...
fn verify_artifact(store: &Path, content_hash: &str) -> AilResult<VerifiedArtifact> {
...
fn resolve_store(store: &Path, root_hash: &str) -> AilResult<(PackageLock, Program)> {
...
fn read_json(path: &Path, maximum: u64, kind: &str) -> AilResult<Value> {
...
fn write_json(path: &Path, document: &Value, code: &'static str) -> AilResult<()> {
...
fn read_bounded(path: &Path, maximum: u64, code: &'static str) -> AilResult<Vec<u8>> {
...
fn canonical_directory(path: &Path, code: &'static str) -> AilResult<PathBuf> {
    let canonical = path.canonicalize().map_err(|error| {
...
fn absolute_path(path: &Path) -> AilResult<PathBuf> {
    if path.is_absolute() {
        Ok(path.to_path_buf())
    } else {
        std::env::current_dir()
