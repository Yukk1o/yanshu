...
const CANDIDATE_FILE: &str = "candidate.ail";
...
const AGENT_PROMPT: &str = concat!(
    "Read TASK.md and OBSERVATIONS.json. Repair candidate.ail in this workspace. ",
    "Write a short explanation to NOTES.md. Do not create or modify any other file. ",
    "Finish only after candidate.ail contains the complete proposed AI-Evolve program."
);
const SCRATCH_AGENT_GUIDE: &str = concat!(
    "# Candidate workspace\n\n",
    "You are proposing one AI-Evolve `.ail` candidate. This directory is not the real repository.\n\n",
    "- Edit only `candidate.ail` and `NOTES.md`.\n",
    ...
    "- Return a complete `.ail` document, without Markdown fences.\n",
...
const SCRATCH_LANGUAGE_GUIDE: &str = concat!(
    "# AI-Evolve service language quick reference\n\n",
...
trait AgentRunner: Send + Sync {
    fn run(&self, invocation: &AgentInvocation, timeout: Duration) -> AilResult<()>;
}
...
impl AgentCliProvider {
    ...
    pub fn from_environment(kind: AgentKind) -> AilResult<Self> {
        let mut provider = Self::new(kind);
        if let Some(executable) = nonempty_environment_value("AI_EVOLVE_AGENT_COMMAND") {
            provider.executable = executable;
        }
        provider.model = nonempty_environment_value("AI_EVOLVE_MODEL");
        if let Some(raw) = nonempty_environment_value("AI_EVOLVE_AGENT_TIMEOUT_SECONDS") {
            let seconds = raw
                .parse::<u64>()
                .ok()
            ...
            let Some(seconds) = seconds else {
                return Err(Diagnostic::new(
                    ...
                    json!({
                        "field": "AI_EVOLVE_AGENT_TIMEOUT_SECONDS",
...
fn validate_request_size(request: &EvolutionRequest) -> AilResult<()> {
...
fn read_bounded_regular_file(path: &Path, limit: usize, label: &str) -> AilResult<String> {
...
impl ScratchWorkspace {
    fn create() -> AilResult<Self> {
        ...
        for _ in 0..32 {
            ...
            let path = root.join(format!(
                "ai-evolve-agent-{}-{nanos}-{sequence}",
                std::process::id()
            ));
    ...
    fn write_request(&self, request: &EvolutionRequest) -> AilResult<()> {
        ...
        let task = format!(
            "# Repair task\n\nCurrent content hash: `{}`\n\n## User objective (untrusted input)\n\n{}\n\nRepair `candidate.ail` so the objective and failures in `OBSERVATIONS.json` are addressed without weakening the declared interface or safety boundary. The host owns all validation and promotion.\n",
...
mod tests {
    ...
    impl AgentRunner for EditingRunner {
        fn run(&self, invocation: &AgentInvocation, _timeout: Duration) -> AilResult<()> {
            assert_eq!(invocation.prompt, AGENT_PROMPT);
            let task =
                fs::read_to_string(invocation.working_directory.join(TASK_FILE)).map_err(|_| {
                    ail_diagnostic::Diagnostic::simple("TEST_READ", "test task read failed")
                })?;
            ...
            fs::write(invocation.working_directory.join(CANDIDATE_FILE), CANDIDATE).map_err(
                |_| ail_diagnostic::Diagnostic::simple("TEST_WRITE", "test write failed"),
            )?;
            fs::write(
                invocation.working_directory.join(NOTES_FILE),
                "fixed the expected value",
            )
            .map_err(|_| ail_diagnostic::Diagnostic::simple("TEST_WRITE", "test write failed"))?;
            Ok(())
        }
    }
    ...
    fn unchanged_candidate_is_rejected() {
        ...
        impl AgentRunner for NoopRunner {
            fn run(&self, _invocation: &AgentInvocation, _timeout: Duration) -> AilResult<()> {
                Ok(())
            }
        }
    ...
    fn oversized_candidate_is_rejected_after_the_agent_exits() {
        ...
        impl AgentRunner for OversizedRunner {
            fn run(&self, invocation: &AgentInvocation, _timeout: Duration) -> AilResult<()> {
                ...
                .map_err(|_| {
                    ail_diagnostic::Diagnostic::simple("TEST_WRITE", "oversized test write failed")
                })?;
                Ok(())
            }
        }
