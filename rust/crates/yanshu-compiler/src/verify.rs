...
pub fn verify_bytecode(artifact: &BytecodeArtifact) -> YanshuResult<()> {
...
fn verify_block(identifier: usize, block: &CodeBlock, block_count: usize) -> YanshuResult<()> {
...
fn validate_name(name: &str) -> YanshuResult<()> {
...
fn check_target(block: usize, pc: usize, target: usize, length: usize) -> YanshuResult<()> {
    if target >= length {
        Err(Diagnostic::new(
...
fn require_stack(state: State, required: usize, block: usize, pc: usize) -> YanshuResult<()> {
...
fn pushed(state: State, block: usize, pc: usize) -> YanshuResult<State> {
    Ok(State {
...
fn popped(state: State, block: usize, pc: usize) -> YanshuResult<State> {
...
fn enqueue(pending: &mut VecDeque<(usize, State)>, pc: usize, state: State) -> YanshuResult<()> {
