...
impl EvolutionProvider for FileProvider {
    ...
    fn propose(&self, _request: &EvolutionRequest) -> YanshuResult<EvolutionProposal> {
...
pub trait JsonTransport: Send + Sync {
    fn post_json(&self, request: TransportRequest<'_>) -> YanshuResult<JsonValue>;
}
    ...
    pub fn new(
        ...
        if api_key.trim().is_empty() {
            return Err(Diagnostic::simple(
                ...
                "set YANSHU_API_KEY, DEEPSEEK_API_KEY, or OPENAI_API_KEY before using a live provider",
            ));
        }
        Ok(Self {
            kind,
...
pub fn configured_live_provider() -> YanshuResult<LiveProvider> {
    configured_live_provider_with(|name| env::var(name).ok(), Arc::new(ReqwestJsonTransport))
}

pub fn configured_evolution_provider() -> YanshuResult<Box<dyn EvolutionProvider>> {
    let selected = env::var("YANSHU_PROVIDER")
        .ok()
...
fn configured_live_provider_with(
    ...
    transport: Arc<dyn JsonTransport>,
) -> YanshuResult<LiveProvider> {
    let explicit_kind = nonempty_environment_value(&lookup, "YANSHU_PROVIDER");
    let configured_base = nonempty_environment_value(&lookup, "YANSHU_BASE_URL");
    let configured_model = nonempty_environment_value(&lookup, "YANSHU_MODEL");
    ...
    let api_key = nonempty_environment_value(&lookup, "YANSHU_API_KEY").or_else(|| match kind {
...
fn evolution_input(request: &EvolutionRequest) -> YanshuResult<String> {
...
) -> YanshuResult<EvolutionProposal> {
...
fn validate_candidate_document(candidate: JsonValue) -> YanshuResult<(String, String)> {
...
mod tests {
    ...
    impl JsonTransport for MockTransport {
        ...
        ) -> yanshu_diagnostic::YanshuResult<JsonValue> {
    ...
    fn require_error<T>(
        result: yanshu_diagnostic::YanshuResult<T>,
    ) -> yanshu_diagnostic::Diagnostic {
        match result {
    ...
    fn environment_selection_is_pure_and_does_not_require_mutating_process_state() {
        let values = [
            ("YANSHU_BASE_URL", "https://api.deepseek.com"),
            ("YANSHU_MODEL", "deepseek-v4-flash"),
            ("YANSHU_API_KEY", "test-key"),
            ("YANSHU_MAX_OUTPUT_TOKENS", "2048"),
        ];
        ...
        let invalid = require_error(super::configured_live_provider_with(
            |name| match name {
                "YANSHU_PROVIDER" => Some("unknown".to_owned()),
                "YANSHU_API_KEY" => Some("test-key".to_owned()),
                _ => None,
            },
            Arc::new(MockTransport::new(json!({}))),
        ));
