...
async fn parse_request(request: Request, config: &HttpConfig) -> YanshuResult<ServiceRequest> {
...
fn decode_path(raw: &str) -> YanshuResult<String> {
    if raw == "/" {
        return Ok("/".to_owned());
    }
    if !raw.starts_with('/') {
...
fn decode_query(raw: &str) -> YanshuResult<BTreeMap<String, GuestValue>> {
    let mut result = BTreeMap::new();
    if raw.is_empty() {
        return Ok(result);
    }
...
fn validate_config(config: &HttpConfig) -> YanshuResult<()> {
