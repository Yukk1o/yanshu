...
pub fn parse_program(datum: &Datum, source: &str) -> YanshuResult<Program> {
...
fn parse_data_field(datum: &Datum) -> YanshuResult<DataField> {
...
fn parse_function_signature(form: &[Datum], datum: &Datum) -> YanshuResult<FunctionSignature> {
    if form.len() != 3 || form[1].symbol().is_none() {
        return Err(at(
            datum,
...
fn parse_type_expression(datum: &Datum, depth: usize) -> YanshuResult<TypeExpression> {
...
fn parse_schema_specification(datum: &Datum, depth: usize) -> YanshuResult<SchemaKind> {
...
fn parse_schema_field(datum: &Datum, depth: usize) -> YanshuResult<SchemaField> {
...
fn parse_expression(datum: &Datum) -> YanshuResult<Expression> {
...
fn parse_cond(form: &[Datum], datum: &Datum) -> YanshuResult<ExpressionKind> {
...
fn parse_match(form: &[Datum], datum: &Datum) -> YanshuResult<ExpressionKind> {
    if form.len() < 3 {
        return Err(Diagnostic::simple(
...
fn parse_pattern(datum: &Datum, bindings: &mut HashSet<String>) -> YanshuResult<Pattern> {
    let kind = match &datum.kind {
...
fn ensure_expression_version(expression: &Expression, version: &BigInt) -> YanshuResult<()> {
...
fn ensure_schema_version(schema: &SchemaKind, version: &BigInt, datum: &Datum) -> YanshuResult<()> {
...
fn validate_route_path(path: &str, datum: &Datum) -> YanshuResult<()> {
    if path.is_empty()
...
mod tests {
    ...
    const UNKNOWN_LIBRARY: &str =
        include_str!("../../../../conformance/v1/invalid/unknown-library.yan");
