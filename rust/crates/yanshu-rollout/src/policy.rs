...
impl ShadowPolicy {
    pub fn new(candidate_version: impl Into<String>, sample_percent: u8) -> YanshuResult<Self> {
...
fn validate_version(version: &str) -> YanshuResult<()> {
    if version.len() == 64
        && version
            .bytes()
...
mod tests {
    ...
    fn require<T>(result: yanshu_diagnostic::YanshuResult<T>) -> T {
