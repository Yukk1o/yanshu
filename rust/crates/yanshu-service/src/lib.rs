...
use yanshu_store::atomic_replace;
use yanshu_syntax::{Program, Route};
...
pub fn run_service_suite(program: &Program, path: impl AsRef<Path>) -> YanshuResult<JsonValue> {
...
fn validate_guest_response(value: Value) -> YanshuResult<ServiceResponse> {
...
fn expect_kv_key<'value>(operation: &str, value: &'value Value) -> YanshuResult<&'value str> {
...
fn json_object_to_guest_map(value: &JsonValue) -> YanshuResult<BTreeMap<String, Value>> {
...
fn json_integer(value: &JsonValue) -> YanshuResult<BigInt> {
    match value {
...
fn read_store_file(path: &Path) -> YanshuResult<BTreeMap<String, Value>> {
...
fn write_store_file(path: &Path, data: &BTreeMap<String, Value>) -> YanshuResult<()> {
    let entries = data
        .iter()
...
mod tests {
    ...
    use yanshu_runtime::Value as GuestValue;
    ...
    const EXPENSE_SERVICE: &str = include_str!("../../../../examples/expenses/service.yan");
    ...
    fn file_kv_survives_reopen_with_canonical_json() {
        ...
        let create = ServiceRequest {
            ...
            body: require(yanshu_runtime::json_to_value(&create_body)),
        };
        ...
        let persisted: JsonValue = require(
            fs::read_to_string(&store_path)
                .map_err(|_| yanshu_diagnostic::Diagnostic::simple("TEST_READ", "read failed"))
                .and_then(|source| {
                    serde_json::from_str(&source).map_err(|_| {
                        yanshu_diagnostic::Diagnostic::simple("TEST_JSON", "json failed")
                    })
                }),
        );
        assert_eq!(
    ...
    fn failed_file_replacement_does_not_commit_memory_or_leave_temporary_files() {
        ...
        let request = ServiceRequest {
            ...
            query: Default::default(),
            headers: Default::default(),
            body: require(yanshu_runtime::json_to_value(&serde_json::json!({
                "id": "must-not-commit",
                "title": "write failure",
                "completed": false
            }))),
